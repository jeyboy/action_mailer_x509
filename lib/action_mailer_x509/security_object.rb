  ...
  class << self
    private
      def subject(attrs={})
        ...
        subject = attrs.each_with_object('') do |attr, obj|
      ...
      def rsa_key(length = 4096, password = nil)
        key = OpenSSL::PKey::RSA.new(length) # the CA's public/private key
        (repack_key(key, password) if password.present?) || key
      end

      def certificate(params, root_certificate = nil)
        root_key = rsa_key(params[:key_length], params[:password])
...
#      cert.sign key, OpenSSL::Digest::SHA1.new
    ...
    def p12_certificate(params = {})
      ...
      p12 = OpenSSL::PKCS12.create(params[:password], 'My Name', key, cert)
